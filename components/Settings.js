...
export default function Settings({ workouts, setWorkouts, weights, setWeights }) {
  const [section, setSection] = useState('perfil'); // perfil | pesos | historial | rutina
  ...
  // Rutina editable (copia local)
  const [rutina, setRutina] = useState(() => {
    try {
      const r = localStorage.getItem('gym-rutina');
      return r ? JSON.parse(r) : MES5;
    } catch { return MES5; }
  });
  ...
  const renderPerfil = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ ...card, border: `1px solid ${C.danger}44` }}>
        ...
        <p style={{ fontSize: 12, color: C.textMuted, marginBottom: 14 }}>Borra todos los datos de la app y empieza desde cero</p>
        <button onClick={() => {
  ...
  const renderPesos = () => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div style={card}>
          <p style={sec}>➕ Añadir / editar peso</p>
          ...
          <button onClick={addWeight} style={{ width: '100%', padding: 12, borderRadius: 10, border: 'none', background: C.accent, color: C.white, fontWeight: 800, fontSize: 13, cursor: 'pointer' }}>
  ...
  const renderRutina = () => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={card}>
          <p style={sec}>✏️ {selectedDay} — {day.exercises.length} ejercicios</p>
            ...
            <div key={idx} style={{ padding: '12px 0', borderBottom: `1px solid ${C.border}` }}>
              ...
              <div style={{ display: 'flex', gap: 7 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <input value={ex.sets} onChange={e => updateExercise(idx, 'sets', e.target.value)}
                    style={smallInp} placeholder="ej: 4×10" />
                </div>
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: 9, color: C.textMuted, marginBottom: 3 }}>DESCANSO</p>
                  <input value={ex.restLabel} onChange={e => updateExercise(idx, 'restLabel', e.target.value)}
                    style={smallInp} placeholder="ej: 90s" />
                </div>
              </div>
            </div>
          ))}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {section === 'perfil'    && renderPerfil()}
      {section === 'pesos'     && renderPesos()}
      {section === 'historial' && renderHistorial()}
      {section === 'rutina'    && renderRutina()}
    </div>
  );
}
